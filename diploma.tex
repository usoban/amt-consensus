...
\begin{figure}
	...
	\end{center}
	\caption{
		"Drevo življenja", prva skica evolucijske zgodovine, narisana s strani 
		Charlesa Darwina leta 1837~\cite{cd}.
	}
...
Na podlagi razdalj, zapisanih v distančni matriki, izbrana metoda v gručo 
...
medseboj tekmujočimi teorijami evolucije izberemo tisto, ki minimizira število 
...
Zamisli za uporabo bayesovega teorema so se pojavile že konec šestdesetih let, 
...
izmed metod ponovnega vzorčenja, npr. jackknife ali bootstrap, s katero vzorčijo 
...
ene metode. Raziskovalci lahko pridobijo več nasprotujočih si teorij o evolucijski 
zgodovini, naj si bo z uporabo različnih metod za računanje filogenetskega drevesa 
ali iz različnih virov podatkov. Ob tem se pojavi potreba po eni teoriji evolucije, 
ki bi kar se da dobro zajemala vse evolucijske dogodke, ki jih nosijo med sabo 
tekmujoče si teorije. To lahko dosežemo z uporabo konsenznih metod.
...
Večina konsenznih metod ignorira dolžine vej v vhodnih drevesih, tudi če so te navedene. 
...
\begin{itemize}
	...
	\item PHYLIP, paket samostojnih programov, ki ponuja izračun filogenetskih dreves 
		  ...
		  verjetja. Poleg tega ponuja programe za ponovno vzorčenje ter program
...
opažanja, da vsak rob drevesa, $e \in E(T_i)$, drevo razdeli na dva dela oz. particiji.
...
potomci. Takemu poddrevesu pravimo tudi klad (angl. clade). Ne bo nas zanimala 
...
ponovno potrebno povdariti, da se pomen posameznih mest v binarnih nizih med 
...
\begin{figure}
	...
	\end{center}
	\caption{
		Graf nekompatibilnosti biznih nizov za vhodna drevesa iz 
...
$I(V, E)$ lahko obstaja več asimetričnih srednjih dreves.
...
\begin{figure}
	...
	\end{center}
	\caption{
		Dve različni največji neodvisni množici za graf nekompatibilnosti iz 
		slike~\ref{img-incompat-graph-example}. Na danem grafu sicer obstajajo 
		štiri največje neodvisne množice.
	}
...
na levi strani slike \ref{img-reconstruct-1}. Tak graf je že drevo, vendar še ne
filogenetsko.

Preden konstruirano drevo postane filogenetsko drevo, moramo razrešiti oznake 
na povezavah. Razreševanja se lotimo po vrsticah matrike $M'$, saj vsaka vrstica 
pravzaprav predstavlja eno taksonomsko enoto. Za vsak $i \in I_i$ najdemo največji 
...
imamo v vhodni množici $T$ veliko dreves. Zato si bomo ogledali dva 
...
Olajša nam delo s formati datotek, kot so BLAST, ClustralW, FASTA ter GenBank
...
modulu {\it Bio.Phylo.Consensus}. Opis parametrov funkcije se nahaja v 
tabeli~\ref{table-amt-head}. Privzeto metoda izračuna točno asimetrično srednje
...
\begin{table}
    ...
    \caption{
        Opis parametrov metode {\it Bio.Phylo.Consensus.amt\_consensus} in 
        vrednosti, ki jih ti lahko zavzamejo.
    }
...
konsenza. Za ta namen smo pripravili nekaj vhodnih množic in jih preizkusili 
glede na razrešenost končnega drevesa ter glede na Robinson-Fouldsovo metriko. 
...
Biopython RF metrika še ni implementirana. Vrednost RF smo izračunali za 
vsak par konsenznega in vhodnega drevesa, ter za vsako konsenzno drevo 
vrednosti RF sešteli.
...
Tabela \ref{table-eval-1} prikazuje razrešenost in RF vrednosti asimetričnega
...
razrešeno je drevo Bi-AMT, ki je sicer glede na RF metriko nekoliko bolj 
...
konsenznem drevesu, tako po topologiji kot glede na razrešenost in RF metriko. 
Preostala drevesa so glede na RF metriko sicer enakovredna, vendar ker drevo 
...
\begin{table}[h!]
	...
	\caption{
	 Razrešenost konsenznih dreves in njihove RF vrednosti za deset filogenetskih dreves
...
drevesa, ki imajo deset taksonomskih enot. Opazimo, da se čas izvajanja z 
večanjem števila vhodnih dreves najhitreje povečuje za aproksimacijsko metodo 
Bi-AMT, medtem ko je natančna metoda primerljiva z aproksimacijsko metodo MM-AMT.
Najhitrejše izvajanje smo dosegli z natančno metodo skupaj z uporabo programa
PMC. Spodnji del grafa, ki je prikazuje izvajalni čas za drevesa z 20 taksonomskimi
enotami, kaže nekoliko drugačno stanje. Izvajalni čas natančne metode pričakovano
prične naraščati eksponentno glede na število vhodnih dreves in takoj preseže vse
ostale. Med preostalimi metodami se razmerja niso spremenila, še vedno je najhitrejša
natančna metoda z uporabo zunanjega programa PMC.

Izračun natančnega asimetričnega srednjega drevesa je brez zunanjega programa PMC 
mogoč le za majhno število taksonomskih enot.  
\clearpage
\begin{figure}[h!]
	\begin{center}
	...
	\end{center}
	\caption{
		Izvajalni časi vseh treh metod za drevesa z 10 in 20 taksonomskimi enotami
...
\begin{figure}[h!]
	\begin{center}
		\fbox{\includegraphics[scale=0.54, clip=true, trim=0 0 0 0.5cm]{gfx/times_10_20_taxons.pdf}}
	\end{center}
	\caption{
...
narašča precej počasneje. Še vedno je najhitrejši izračun točnega asimetričnega
srednjega drevesa s pomočjo programa PMC.   
...
kompleksnost metode v splošnem eksponentna, kar je nezaželjeno, in se problemi z
izvajalnim časom za natančno metodo pojavijo hitro, ima uporabnik možnost namestitve 
zunanjega programa PMC, ki močno pospeši izvajanje. Kljub temu je za velike vhodne 
množice dreves ($n > 100$) izračun lahko dolgotrajen, čeprav to ne velja v splošnem,
...
uporabnik naleti na težave s časom izvajanja, ima na voljo izbrati eno izmed 
...
izračunali konsenzna drevesa in izbrali najboljšega. Prav tako so potrebne dodatne 
študije, s katerimi bi identificirali lastnosti dreves vhodne množice, za katere da 
metoda asimetričnega srednjega drevesa boljše rezultate.
